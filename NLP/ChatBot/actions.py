...
class GetAllSearchResults(FormAction):
	""" Take all the restaurants for a particular location and cuisine using Zomato APIs """
	RANDOMIZE = False
	@staticmethod
	def required_fields():
		return [
	...
	def submit(self, dispatcher, tracker, domain):
		...
		dispatcher.utter_message("Fetching the results for request ..............")
		fetchIndex=0
...
class ActionSearchRestaurants(Action):
	""" Filters the restaurants as per the budget selected by user and shows the results """
	...
	def run(self, dispatcher, tracker, domain):
		...
		if(price_range=="1"):
			...
			if(len(filterd_results)==0):
				...
				response=response+" Sorry couldn't find any restaurants in the given price range. Please re enter a different price range "
				print(response)
			else:
				...
				print("Found following restaurants...")
				print(response)
...
class ActionValidateLocation(Action):
	""" Validates if the city provided by the user is a Tier1 or Tier2 city.
	If not it notifies user that the location is invalid """
	def name(self):
...
class ActionValidateCuisine(Action):
	""" Validates that the User has selected only from the available cuisines.
...
class ActionSendEmail(Action):
	""" If user wants to receive the restaurrant details via Email, Email is sent """
	def name(self):
		return 'action_email'
	...
	def run(self, dispatcher, tracker, domain):
		...
		msg['From'] = 'chatbot798@gmail.com'
		msg['To'] = email

		server = smtplib.SMTP_SSL('smtp.gmail.com', 465)
		server.ehlo()
		server.login('chatbot798@gmail.com','July2019')
...
class ActionValidatePrice(Action):
	""" Validates if the user has selected budget among the options shown """
	def name(self):
